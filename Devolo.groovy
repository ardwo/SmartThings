...
metadata {
	definition (name: "Devolo X", namespace: "ardwo", author: "Tom Philip") {
		capability "Actuator"
		capability "Sensor"
		...
		//capability "Switch"
        capability "Temperature Measurement"	//ADW
        ...
        attribute "temperature", "number" //ADW
		...
		fingerprint type: "0804", mfr: "0002", prod: "0005", model: "0175"
		fingerprint cc: "80,46,81,72,8F,75,31,43,86,84", ccOut: "46,81,8F" //ADW:dodałem 31
	...
	tiles(scale: 2) {
        ...
        valueTile("temperature", "device.temperature", decoration: "flat", inactiveLabel: false, width: 4, height: 1) {
     		state "actual", label:'Room temperature\n${currentValue}°', unit:"°C"
    	}
       	valueTile("temperature2", "device.temperature", inactiveLabel: false) {
            state "temperature", label:'${currentValue}°', unit: "dC", icon:"st.Weather.weather2", backgroundColors:[
		...
		main "temperature2"
		details(["richtemp", "thermostatMode", "lastWakeUpTile", "temperature"])
	}
	...
	preferences {
		...
		input "quickOnTemperature", "number", title: "Quick On Temperature. Default 21°C.", description: "Quickly turn on the radiator to this temperature", range: "8..82", displayDuringSetup: false
		input "quickOffTemperature", "number", title: "Quick Off Temperature. Default 4°C.", description: "Quickly turn off the radiator to this temperature", range: "8..68", displayDuringSetup: false
	}
}
...
def zwaveEvent(physicalgraph.zwave.commands.wakeupv2.WakeUpNotification cmd) {
	...
	if (thermostatMode == "off") {
		nextHeatingSetpoint = quickOffTemperature ?: fromCelsiusToLocal(8)
	}
...
def temperatureDown() {
	...
	if(nextTemp < fromCelsiusToLocal(8)) {
		nextTemp = fromCelsiusToLocal(8)
		off()
	}
	else {
		on()
...
def on() {
	...
	def offTemperature = quickOffTemperature ?: fromCelsiusToLocal(8)
